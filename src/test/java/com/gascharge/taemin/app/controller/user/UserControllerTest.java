...
public class UserControllerTest {
    ...
    void getUserInfoWithAuthorization() throws Exception {
        ...
        // when
        when(userService.findById(any())).thenReturn(getUserServiceResponseDto(getCloneUser()));
        ...
        mvc.perform(get("/user/me"))
                ...
                .andExpect(jsonPath("$.userAuthority").value(UserAuthority.ROLE_USER.toString()))
    ...
    void getUserListWithEmail() throws Exception {
        ...
        this.mvc.perform(get("/user")
                ...
                .andExpect(jsonPath("$.content[0].image").value(cloneUser.getImageUrl()))
...
//                .andExpect(jsonPath("$.content[0].authProvider").value(cloneUser.getProvider().toString()))
                .andExpect(jsonPath("$.content[0].userAuthority").value(cloneUser.getUserAuthority().toString()))
...
//                .andExpect(jsonPath("$.content[0].authProvider").value(cloneUser.getProvider().toString()))
    ...
    void getUserListWithPageable() throws Exception {
        ...
        this.mvc.perform(get("/user")
                ...
                .andExpect(jsonPath("$.content[0].image").value(USER_TEST_IMAGE_URL))
...
//                .andExpect(jsonPath("$.content[0].authProvider").value(USER_TEST_AUTH_PROVIDER.toString()))
                .andExpect(jsonPath("$.content[0].userAuthority").value(USER_TEST_USER_AUTHORITY.toString()))

                .andExpect(jsonPath("$.content[1].name").value(ADMIN_TEST_NAME))
                .andExpect(jsonPath("$.content[1].email").value(ADMIN_TEST_EMAIL))
                .andExpect(jsonPath("$.content[1].image").value(ADMIN_TEST_IMAGE_URL))
...
//                .andExpect(jsonPath("$.content[1].authProvider").value(ADMIN_TEST_AUTH_PROVIDER.toString()))
                .andExpect(jsonPath("$.content[1].userAuthority").value(ADMIN_TEST_USER_AUTHORITY.toString()))
